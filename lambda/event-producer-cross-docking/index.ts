...
export const handler = async (event: any) => {
  ...
  try {
    ...
    metricsLogger.putDimensions({ Service: "CrossDocking" });
    metricsLogger.putMetric("CrossDocking", 1);
    ...
    if (Array.isArray(masterPlanSteps) && masterPlanSteps.length > 0) {
      expected_location = masterPlanSteps[2]?.expected_location;
      expected_time = masterPlanSteps[2]?.expected_time;
    } else {
    ...
    const eventsToSend = [
      { detailType: "ParcelEvent-CROSS-DOCKING", detail: enrichedEvent },
      // { detailType: "TrackTraceEvent", detail: trackTraceEvent },
      // { detailType: "DigitalLabelEvent", detail: digitalLabelEvent },
    ];

    console.log(
      "Sending events from CrossDocking to eventManagementLambda:",
      eventsToSend,
    );
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: "Events sent from CrossDocking to Event Management Layer",
      }),
    };
  } catch (error) {
