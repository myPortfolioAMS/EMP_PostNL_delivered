...
export const handler = async (event: any) => {
  ...
  try {
    ...
    metricsLogger.putDimensions({ Service: "SECOND-SORTING" });
    metricsLogger.putMetric("SECOND-SORTING", 1);
    ...
    if (Array.isArray(masterPlanSteps) && masterPlanSteps.length > 0) {
      expected_location = masterPlanSteps[3]?.expected_location;
      expected_time = masterPlanSteps[3]?.expected_time;
    } else {
    ...
    const eventsToSend = [
      { detailType: "ParcelEvent-SECOND-SORTING", detail: enrichedEvent },
      // { detailType: "TrackTraceEvent", detail: trackTraceEvent },
      // { detailType: "DigitalLabelEvent", detail: digitalLabelEvent },
    ];

    console.log(
      "Sending events from Second Sorting to eventManagementLambda:",
      eventsToSend,
    );
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: "Events sent from SECOND-SORTING to Event Management Layer",
      }),
    };
  } catch (error) {
