...
export const handler = async (event: any) => {
  ...
  try {
    ...
    metricsLogger.putDimensions({ Service: "COLLECTION" });
    metricsLogger.putMetric("COLLECTION", 1);
    ...
    if (Array.isArray(masterPlanSteps) && masterPlanSteps.length > 0) {
      expected_location = masterPlanSteps[0]?.expected_location;
      expected_time = masterPlanSteps[0]?.expected_time;
    } else {
    ...
    const eventsToSend = [
      { detailType: "ParcelEvent-COLLECTION", detail: enrichedEvent },
      // { detailType: "TrackTraceEvent", detail: trackTraceEvent },
      // { detailType: "DigitalLabelEvent", detail: digitalLabelEvent },
    ];

    console.log(
      "Sending events from COLLECTION to eventManagementLambda:",
      eventsToSend,
    );
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: "Events sent from COLLECTION to Event Management Layer",
      }),
    };
  } catch (error) {
