...
export const handler = async (event: any) => {
  ...
  try {
    ...
    metricsLogger.putDimensions({ Service: "DISTRIBUTION" });
    metricsLogger.putMetric("DISTRIBUTION", 1);
    ...
    if (Array.isArray(masterPlanSteps) && masterPlanSteps.length > 0) {
      expected_location = masterPlanSteps[4]?.expected_location;
      expected_time = masterPlanSteps[4]?.expected_time;
    } else {
    ...
    const eventsToSend = [
      { detailType: "ParcelEvent-DISTRIBUTION", detail: enrichedEvent },
      // { detailType: "TrackTraceEvent", detail: trackTraceEvent },
      // { detailType: "DigitalLabelEvent", detail: digitalLabelEvent },
    ];

    console.log(
      "Sending events from Distribution to eventManagementLambda:",
      eventsToSend,
    );
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: "Events sent from Distribution to Event Management Layer",
      }),
    };
  } catch (error) {
