...
export const handler = async (event: any) => {
  ...
  try {
    ...
    metricsLogger.putDimensions({ Service: "FINAL-CONSUMER" });
    metricsLogger.putMetric("FINAL-CONSUMER", 1);
    ...
    if (Array.isArray(masterPlanSteps) && masterPlanSteps.length > 0) {
      expected_location = masterPlanSteps[5]?.expected_location;
      expected_time = masterPlanSteps[5]?.expected_time;
    } else {
    ...
    const enrichedEvent = {
      ...
      data: requestBody, // ✅ Store the original event inside `data`
    };
    ...
    const eventsToSend = [
      { detailType: "ParcelEvent-FINAL-CONSUMER", detail: enrichedEvent },
      // { detailType: "TrackTraceEvent", detail: trackTraceEvent },
      // { detailType: "DigitalLabelEvent", detail: digitalLabelEvent },
    ];

    console.log(
      "Sending events from final-consumer to eventManagementLambda:",
      eventsToSend,
    );
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: "✅ Events sent from final-consumer to Event Management Layer",
      }),
    };
  } catch (error) {
